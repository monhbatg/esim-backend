...
export class AdminService {
  ...
  async getDashboard(): Promise<{
    ...
    const allOrder = await this.esimInvoiceRepo.count();
    const complatedOrder = await this.esimPurchaseRepo.count();
    const incompated = allOrder - complatedOrder; 
    ...
    const newCustomersThatMonth = await this.customerRepository.count({
      where: {
        createdAt: Between(startOfMonth, startOfNextMonth),
      },
    });

    const result = await this.esimPurchaseRepo
    .createQueryBuilder("purchase")
    .select("COUNT(DISTINCT purchase.customerId)", "count")
    .getRawOne();

    const activeCustomersCount = Number(result.count);

    // 2 or more times buyer
    const result1 = await this.esimPurchaseRepo
  .createQueryBuilder("purchase")
  .select("purchase.customerId")
  .groupBy("purchase.customerId")
  .having("COUNT(purchase.customerId) >= :min", { min: 2 })
  .getRawMany();

      const mostBuyer = Number(result1.length);

    return {
      allTimeOrders: {
      count: allOrder,
      complated: complatedOrder,
      incompated: incompated
      },
      allCustomers: {
        totalCustomer: totalCustomers,
        activeCustomer: activeCustomersCount,
        thisMonthNew: newCustomersThatMonth,
        mostBuyer: mostBuyer
      }
    }
  }
...
function getSalaryDate(): {start: any; end: any; title: string}{
  const today = new Date();
    ...
    const dayOfMonth = ulaanbaatarDate.getDate();
